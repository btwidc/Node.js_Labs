...
let http_handler = (req, res) => {
  ...
  if (url.parse(req.url).pathname === '/connection') {
    if (!url.parse(req.url, true).query.set) {
      ...
      res.end(`<h3>New value of KeepAliveTimeout: ${server.keepAliveTimeout}</h3>`);
    }
  }
  ...
  if (url.parse(req.url).pathname === '/parameter') {
    if ((x = +url.parse(req.url, true).query.x) && (y = +url.parse(req.url, true).query.y)) {
      ...
      res.end(`<h3>${url.parse(req.url).pathname}</h3> \n <h3>Parameters is not number</h3>`);
    }
  }
  ...
  if (url.parse(req.url).pathname === '/socket') {
    ...
    res.end(`<h2>Local Address: ${req.socket.remoteAddress} \n Local Port: ${req.socket.remotePort} \n Server Address: ${res.socket.localAddress} \n Server Port: ${res.socket.localPort}</h2>`);
  }
  ...
  if (url.parse(req.url).pathname === '/json' && req.method === 'POST') {
    ...
    req.on('end', () => {
      ...
      let jsonResponse = {};
      jsonResponse.__comment = 'Ответ. Лабораторная работа 8';
